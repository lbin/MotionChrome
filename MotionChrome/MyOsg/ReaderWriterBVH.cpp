    ...
    void buildHierarchy( osgDB::Input& fr, int level, osgAnimation::Bone* parent )
    {
        ...
        if ( fr.matchSequence("CHANNELS %i") )
        {
            ...
            if(noChannels == 6){
                ...
                {
                    fr.readSequence( channelName );
                    if (channelName == "Xrotation") str+="X";
                    else if (channelName == "Yrotation") str+="Y";
                    else if (channelName == "Zrotation") str+="Z";
                }
                if (str == "XYZ") _joints.back().second = 0xF;
                if (str == "XZY") _joints.back().second = 0x17;
                if (str == "YXZ") _joints.back().second = 0x1F;
                if (str == "YZX") _joints.back().second = 0x27;
                if (str == "ZXY") _joints.back().second = 0x2F;
                if (str == "ZYX") _joints.back().second = 0x37;
            }
            if(noChannels == 3){
                string str = "";
                ...
                if (str == "XYZ") _joints.back().second = 0x08;
                if (str == "XZY") _joints.back().second = 0x10;
                if (str == "YXZ") _joints.back().second = 0x18;
                if (str == "YZX") _joints.back().second = 0x20;
                if (str == "ZXY") _joints.back().second = 0x28;
                if (str == "ZYX") _joints.back().second = 0x30;
            }
        }
...
protected:
                      ...
                      osgAnimation::QuatKeyframeContainer* rotKey )
    {
        ...
        if ( rotKey )  // Rotation keyframe
        {
            float keyValue[3] = { 0.0f };
            osg::Vec3 x,y,z;
            x = osg::Vec3(1.0,0.0,0.0);
            y = osg::Vec3(0.0,1.0,0.0);
            z = osg::Vec3(0.0,0.0,1.0);
            osg::Vec3 v1,v2,v3;
            if ( ch == 0x08 ) {v1 = z; v2 = y; v3 = x;}
            if ( ch == 0x10 ) {v1 = y; v2 = z; v3 = x;}
            if ( ch == 0x18 ) {v1 = z; v2 = x; v3 = y;}
            if ( ch == 0x20 ) {v1 = x; v2 = z; v3 = y;} 
            if ( ch == 0x28 ) {v1 = y; v2 = x; v3 = z;} 
            if ( ch == 0x30 ) {v1 = x; v2 = y; v3 = z;}  
            fr.readSequence( keyValue[0] );
            fr.readSequence( keyValue[1] ); 
            fr.readSequence( keyValue[2] );
            ...
            osg::Matrix rotMat = osg::Matrix::rotate(osg::DegreesToRadians(keyValue[2]), v1)
                               * osg::Matrix::rotate(osg::DegreesToRadians(keyValue[1]), v2)
                               * osg::Matrix::rotate(osg::DegreesToRadians(keyValue[0]), v3);
